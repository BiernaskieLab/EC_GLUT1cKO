...
# Returns:
    ...
    for i in range(ion_count):
        for j in range(resample):
            ...
            col_means[i,j] = np.mean(random.sample(population=sorted(input.iloc[i,1:]), k=int(len(input.iloc[i,1:])/resample)))
...
def diff_compare_split(reference, input):
    ...
    for i in range(np.shape(input)[0]):
        ...
        b = ((np.mean([input[i,0], input[i,3]*0.6])-np.mean(reference[i,3])) / np.mean(reference[i,3]))
...
def stat_compare_split(input):
    ...
    for i in range(ion_count):
        output[i,14:16] = (input[i,14:16]*0.6)-np.mean(input[i,6:8])
...
# Ventricle
# Data Org (Tuple)
ventricle_all = [corpus_c1, corpus_c2, corpus_c3, corpus_c4, corpus_ko1, corpus_ko2, corpus_ko3, corpus_ko4]
ventricle_ally = [corpus_c1y, corpus_c2y, corpus_c3y, corpus_ko1y, corpus_ko2y, corpus_ko3y]
...
diff_ventricle = diff_compare_split(dist_ventricle[:,0:4], dist_ventricle[:,4:8])
diff_ventricley = diff_compare_local(dist_ventricley[:,0:3], dist_ventricley[:,3:6])
...
fig, ax = plt.subplots()
ax.scatter(x=1-(1/np.power(10,diff_ventricle)), y=-np.log10(stat_ventricle), s=16, color=colors, label=None)
...
plt.axvline(-0.15,color="grey",linestyle="--")
plt.axvline(0.15,color="grey",linestyle="--")
...
for i in index:
    if -np.log10(stat_ventricle[i]) > 0.75:
        plt.text(x=1-(1/np.power(10,diff_ventricle[i])), y=-np.log10(stat_ventricle[i]), s=name_annot[i], fontsize=5)

fig, ax = plt.subplots()
ax.scatter(x=1-(1/np.power(10,diff_ventricley)), y=-np.log10(stat_ventricley), s=16, color=colors, label=None)
ax.scatter(x=0, y=0, color='darkred', s=16, label='Fatty Acyls')
ax.scatter(x=0, y=0, color='darkmagenta', s=16, label='Sterols')
ax.scatter(x=0, y=0, color='slategray', s=16, label='Sphingolipids')
ax.scatter(x=0, y=0, color='black', s=16, label='Glycerophospholipids')
...
for i in index:
    if -np.log10(stat_ventricley[i]) > 0.75:
        plt.text(x=1-(1/np.power(10,diff_ventricley[i])), y=-np.log10(stat_ventricley[i]), s=name_annot[i], fontsize=5)
