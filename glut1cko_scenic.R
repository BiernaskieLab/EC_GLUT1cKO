...
setwd("/work/biernaskie_lab/apun/nilesh_glut1/seurat")
load("GLUTKO_rename_olig.Robj", verbose = TRUE)
oligclus_glut <- GLUTKO
Idents(oligclus_glut) <- "gender"
oligclus_glut_females <- subset(oligclus_glut, idents = "Female")
...
oligclus_glut_females[["AUC"]] <- AUC_assay
DefaultAssay(oligclus_glut_females) <- "AUC"
setwd("/work/biernaskie_lab/apun/nilesh_glut1/scenic")
save(oligclus_glut_females,file="fem_oligclus_glutko_scenic_seurat.Robj")

load("fem_oligclus_glutko_scenic_seurat.Robj", verbose=TRUE)

setwd("/work/biernaskie_lab/apun/nilesh_glut1/scenic/deg")
#DEG
DefaultAssay(oligclus_glut_females) <- "AUC"
for (cell_type in unique(oligclus_glut_females$cell_ident)) {
	for(age in unique(oligclus_glut_females$age)) {
		if (cell_type %in% c("Oligodendrocytes-1","Oligodendrocytes-2","Oligodendrocytes-3","Oligodendrocytes-4")) {
			Idents(object = oligclus_glut_females) <- "age"
			females_age <- subset(oligclus_glut_females, idents = age)
